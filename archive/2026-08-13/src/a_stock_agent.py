...
def fetch_macro_data(macro_config):
    ...
    for country, indicator in search_pairs:
        query = urllib.parse.quote(f"{country} {indicator} 最新数据 {datetime.now().strftime('%Y年%m月')}")
        url = f"https://www.bing.com/news/search?q={query}&format=rss"
...
def fetch_event_factors(factors_config):
    ...
    for category, keyword in search_pairs:
        query = urllib.parse.quote(f"{keyword} 最新动态 {datetime.now().strftime('%Y年%m月')}")
        url = f"https://www.bing.com/news/search?q={query}&format=rss"
        text = fetch_url(url, timeout=8)
        if not text:
            continue
...
def fetch_national_team(nt_config):
    ...
    for keyword in ["汇金 ETF 增持", "国家队 ETF 净流入", "社保基金 加仓"]:
        query = urllib.parse.quote(f"{keyword} 最新 {datetime.now().strftime('%Y年%m月')}")
        url = f"https://www.bing.com/news/search?q={query}&format=rss"
        text = fetch_url(url, timeout=8)
        if not text:
            continue
...
def main():
    ...
    if "--backtest" in sys.argv:
        ...
        if data:
            ...
            out = Path(__file__).parent / f"回测报告-{datetime.now().strftime('%Y%m%d')}.html"
    ...
    today = datetime.now().strftime("%Y%m%d")
    output_path = OUTPUT_DIR / f"A股舆情操作指引-{today}.html"
