...
功能：
  ...
  2. 用东方财富日K线接口回测该判断在未来 1/3/5 个交易日后的实际涨跌幅与方向命中率
...
def seed_judgments(report_dir: Path = BASE_DIR):
    """解析工作区内所有报告 HTML，写入 stock_judgments（去重）"""
    pats = ["A股操作指引*9章节*.html", "A股舆情操作指引*9维度*.html", "A股操作指引*9章节*.html"]
    files = []
    for p in pats:
        files += list(report_dir.glob(p))
    # 去重
    seen, all_j = set(), []
    ...
    for w in WINDOWS:
        ...
        if b.get("rate") is None:
            ...
            wrows += (f"<tr><td>{w}日</td><td>{b['hits']}/{b['n']}</td>"
                      f"<td><b class='{cls_of(b['rate']>=50)}'>{b['rate']}%</b></td></tr>")

    # 按个股
    srows = ""
    for code, s in sorted(agg["by_stock"].items()):
        srows += (f"<tr><td>{code}</td><td>{s['hits']}/{s['n']}</td>"
                  f"<td><b class='{cls_of(s['rate']>=50)}'>{s['rate']}%</b></td>"
...
<div class="sub">每日报告个股操作指引 → 东方财富日K实测收益 → 独立技术信号对照</div>
...
def main():
    ...
    if args.run or args.all:
        ...
        out = BASE_DIR / f"回测报告-{datetime.now().strftime('%Y%m%d')}.html"
